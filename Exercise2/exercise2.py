...
def ComputeGradients(image):
  ...
  # Define the kernels
  dxKernel = np.asarray([[-1, 1]])
  dyKernel = np.asarray([[-1], [1]])
  dx2Kernel = np.asarray([[1, -2, 1]])
  dy2Kernel = np.asarray([[1], [-2], [1]])
  laplacianKernel = np.asarray([[-1, -2, -1], [-2, 12, -2], [-1, -2, -1]])
...
def ComputeEdges(dx, dy, gradientMagnitude):
  ...
  # Threshold the images at magnitude over 0.075.
  
  dx = convertToMag(dx)
  dy = convertToMag(dy)  

  ret, dxEdges = cv2.threshold(dx, 0.075, 1.0, cv2.THRESH_BINARY)
  ret, dyEdges = cv2.threshold(dy, 0.075, 1.0, cv2.THRESH_BINARY)
  ret, gradientMagnitudeEdges = cv2.threshold(gradientMagnitude, 0.075, 1.0, cv2.THRESH_BINARY)
...
def DifferenceOfGaussiansLaplacian(gauss1, gauss2, gauss3):
  ...
  Laplacian1 = cv2.Laplacian(gauss1, cv2.CV_32F)
  Laplacian2 = cv2.Laplacian(gauss2, cv2.CV_32F)
...
if __name__ == '__main__':
  ...
  if dy2_student.shape == dy2_correct.shape and np.all(np.abs(dy2_student - dy2_correct) < 0.01):
    ...
    if dy2_student.shape != dy2_correct.shape:
      ...
      print "    You are using 'a' directional edge detector, but not the second finite difference operator. Read the notes carefully."    
  # 2.1.a laplacian
  laplacian_student = cv2.imread(os.path.expanduser("laplacian.png"), 0) / 255.0
  laplacian_correct = cv2.imread(os.path.expanduser("answer-images/laplacian.png"), 0) / 255.0
  if laplacian_student.shape == laplacian_correct.shape and np.all(np.abs(laplacian_student - laplacian_correct) < 0.01):
    print "Exercise 2.1.a: laplacian CORRECT"
  else:
