...
def padded_convolution(image, kernel, verbose=False):
  ...
  image = cv2.copyMakeBorder(image, mend, mend, nend, nend, cv2.BORDER_WRAP)
  
  image = basic_convolution(image, kernel, verbose=True)
  
  ## Return the centred result
  image = image[1.5*mend:1.5*mend+iend, 1.5*nend:1.5*nend+jend]
...
def basic_convolution_dft(image, kernel, verbose=False):
  ...
  dft_kernel = np.fft.fft2(kernel, s = [iend, jend])
  dft_image = np.fft.fft2(image, s = [iend, jend])
  dft_output = np.zeros(dft_image.shape, 'complex')
  
  for i in range(0, iend):
    for j in range(0, jend):
        dft_output[i][j] = dft_kernel[i][j]*dft_image[i][j]
  
  image = np.fft.ifft2(dft_output).astype(float)
...
def padded_convolution_dft(image, kernel, verbose=False):
  ...
  image = cv2.copyMakeBorder(image, mend, mend, nend, nend, cv2.BORDER_WRAP)
  image = basic_convolution_dft(image, kernel, verbose=True)
  image = image[1.5*mend:1.5*mend+iend, 1.5*nend:1.5*nend+jend]
